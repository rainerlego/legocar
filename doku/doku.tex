...
We added a programming socket to the board, so that the microcontroller does not be removed while flashing, but can be programmed onboard.
The 12Mhz crystal oszillator is needed, because the internal atmega8 oscillator only has 8Mhz and is not as precise as the external 12MHz oszillator.
...
Each of them is directly connected to a ...
